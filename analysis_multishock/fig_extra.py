...
def main() -> None:
    ...
    # --- Rule-year sensitivity (M4): benefit-income base at 2022 rules -----
    ...
    bases = {}
    for ry in (2022, 2023):
        b = sum(np.asarray(sim.calculate(v, ry, map_to="household"))
                for v in UPRATED_CASH)
        bases[ry] = weighted(b, w) / 1e9
    shortfall = {ry: bases[ry] * gap for ry in bases}
    ...
    with open("../paper_multishock/generated_extra.tex", "w") as f:
        ...
        for k, v in [("SsBenefitIncomeBnRuleTwentyTwo", bases[2022]),
                     ("SsShortfallBnRuleTwentyTwo", shortfall[2022]),
                     ("SsWithinLo", within_lo), ("SsWithinHi", within_hi),
