...
reported below omit a \HMRCAnticipationSpec, covering both the announcement
month and the preceding quarter, and retain the April-only rule as a
specification check so the two conventions can be compared. The primary WLS weights
...
effect detectable at conventional power is \HMRCHighTariffMDE\ in logs,
against an estimate of \HMRCHighTariffEstimateLog. A second, independent
...
effect. The stored diagnostics now record each group's effective product
count, standard error, confidence interval, interval width and minimum
detectable effect, together with a formal test of the high-minus-other
difference---which does not reject
($p=\HMRCGroupDifferencePValue$). Every power figure in this paragraph is
read from that artifact rather than derived by hand.
