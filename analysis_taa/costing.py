...
PARAMS = {
    ...
    # ISERBS-parameterised cell (Iron and Steel Employees Re-adaptation
...
def latex_macros(res, ashe, flows, red02_detail, mods):
    ...
    # --- Referee-fix addendum (report R2, Aug 2026) -------------------------
    # A+B+C designed-transfer totals excluding the Arm D exposure bound,
    # which is a bound, not a costing, and must not anchor headline sums.
    _dkey = "uc_capital_disregard_bound"
    for _e, _lab in (("narrow", "Narrow"), ("broad", "Broad"),
                     ("exposed", "Exposed")):
        for _s, _slab in (("low", "Low"), ("central", "Central"),
                          ("high", "High")):
            _v = res["totals"][_e][_s] - res["arms"][_dkey][_e][_s]
            add(f"TotalABC{_slab}{_lab}".replace("Taa", ""), fmt(_v))
    _abc_cn = res["totals"]["narrow"]["central"] \
        - res["arms"][_dkey]["narrow"]["central"]
    # Steel comparison on an annual basis: the ~GBP479m of commitments span
    # roughly two years (Port Talbot board from Sep 2024, Scunthorpe from
    # Apr 2025), so per-year ~GBP240m -- a labelled convention.
    _steel_py = res["cross_checks"]["steel_support_m"] / 2.0
    add("SteelPerYear", fmt(_steel_py))
    add("ABCMultipleSteelPerYear", fmt(_abc_cn / _steel_py, 1))
    # Flow-noise variant: annualise the three non-minimum quarters (the
    # Apr-Jun 2026 manufacturing cell is anomalously low).
    _q = res["inputs"]["redundancy_flows"]["quarters"]
    _mvals = sorted(v["manufacturing"] for v in _q.values())
    _exanom = sum(_mvals[1:]) * 4.0 / 3.0
    _cur = sum(_mvals)
    add("FlowManufExAnom", fmt(_exanom))
    add("TotalABCCentralNarrowExAnom", fmt(_abc_cn * _exanom / _cur))
    add("FlowRatioGFCExAnom",
        fmt(res["modules"]["stress"]["stress_manuf_flows"]["gfc"] / _exanom, 1))
    # Full-entitlement break-even: Arm A pays for TWO years, so the
    # per-worker outlay is twice the annual figure; the JSA saving applies
    # only to the first six months of the counterfactual spell (UC-only
    # thereafter).  The headline 3.6 is per year of payment.
    _bk = res["modules"]["breakeven"]
    _cost2 = 2.0 * _bk["wage_insurance_per_worker_per_yr"]
    _jsa_m = 95.55 * 52 / 12.0
    _uc_only = _bk["benefit_saving_per_month"] - _jsa_m
    _after = _uc_only + _bk["tax_nic_per_month"]
    _full = 6.0 + (_cost2 - 6.0 * _bk["fiscal_value_per_month"]) / _after
    add("BreakEvenMonthsFull", fmt(_full, 1))
