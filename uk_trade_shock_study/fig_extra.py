...
def main() -> None:
    ...
    # NOTE: an earlier version computed "2022-rules" benefit aggregates by
    # calculating at period 2022 on the raw dataset. The enhanced FRS is
    # single-period (2023), so those calculations ran on empty income
    # inputs and were meaningless; they are removed. The genuine
    # rulebook-vintage run lives in vintage_and_tca.py, which copies every
    # input column to the 2022 period first.

    # --- Macros ------------------------------------------------------------
    with open(RESULTS_DIR / "generated_extra.tex", "w") as f:
        f.write("% generated by fig_extra.py -- do not edit by hand.\n")
        f.write(f"\\newcommand{{\\SsBootReps}}{{{N_BOOT}}}\n")
        names = ["One","Two","Three","Four","Five"]
        for i, nm in enumerate(names):
            f.write(f"\\newcommand{{\\SsNetLoDec{nm}}}{{{net_lo[i]:,.0f}}}\n")
            f.write(f"\\newcommand{{\\SsNetHiDec{nm}}}{{{net_hi[i]:,.0f}}}\n")
    ...
    import shutil
    shutil.copy(str(RESULTS_DIR / "generated_extra.tex"), str(PAPER_DIR / "generated_extra.tex"))
